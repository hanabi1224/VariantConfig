...
mod tests {
    ...
    use variant_config::dsl::{FnJitter, ParameterValue};

    #[test]
    fn test_str_1() {
        let mut ctx = HashMap::new();
        // ctx.insert("a".to_owned(), ParameterValue::Int(0));
        ctx.insert("a".to_owned(), ParameterValue::String("what".to_owned()));
    ...
    fn test_str_2() {
        let mut ctx = HashMap::new();
        // ctx.insert("a".to_owned(), ParameterValue::Int(0));
        ctx.insert("a".to_owned(), ParameterValue::String("no".to_owned()));
        const CODE: &str = "a == 'what'";
        let jitter = FnJitter::new(CODE).unwrap();
        let ret = jitter.evaluate(&ctx);
        assert_eq!(ret, false);
    ...
    fn test_int_1() {
        let mut ctx = HashMap::new();
        ctx.insert("a".to_owned(), ParameterValue::Int(10));
        const CODE: &str = "a > 8 ";
        let jitter = FnJitter::new(CODE).unwrap();
        let ret = jitter.evaluate(&ctx);
        assert_eq!(ret, true);
    ...
    fn test_add_1() {
        let mut ctx = HashMap::new();
        ctx.insert("a".to_owned(), ParameterValue::Int(10));
        ctx.insert("b".to_owned(), ParameterValue::Int(3));
